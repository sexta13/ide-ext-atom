...
Api.prototype.sendRequest = function (method, url, form, token, callback) {
    ...
    if (method === 'POST') {
        var x=form.getBuffer();
        r.send(form.getBuffer())
    } else {
        r.setRequestHeader('Content-Type', 'application/json')
        r.send(JSON.stringify(form))
    }
...
// send put request
Api.prototype.put = function (url, form, token, callback) {
    this.sendRequest('PUT', url, form, token, callback);
}

// send delete request
Api.prototype.delete = function (url, token, callback) {
