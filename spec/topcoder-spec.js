...
describe('TopcoderWorkflow E2E tests', () => {
  ...
  beforeEach(() => {
    ...
    nock(/\.com/)
      ...
      .reply(403)
      .post(testConfig.TC.REGISTER_API_PATH.replace('{challengeId}', '12345'))
      .reply(200, {})
  ...
  it('begin to view open challenges', () => {
    ...
    runs(() => {
      expect(atom.workspace.getActivePaneItem().getElement().innerHTML.startsWith(`<table border="1">`)).toBe(true)
  ...
  it('begin to view open challenges without login', () => {
    ...
    runs(() => {
      ...
      expect(activePane.innerHTML.startsWith(`<table border="1">`)).toBe(true)
  ...
  it('begin to view challenge detail', () => {
    const rows = activePane.children[0].rows
    let index
  ...
  // test register in challenge
  it('register with success in a challenge', () => {
    const rows = activePane.children[0].rows
    let index
    for (index = 0; index < rows.length; index++) {
      if (rows[index].cells.item(1).textContent === 'DESIGN') {
        break
      }
    }
    ...
    }, 'Timeout issue when registering in challenge', testConfig.TIMEOUT)
    runs(() => {
      const button = atom.workspace.getActivePaneItem().getElement().getElementsByTagName('button')[0]
      button.click()

      waitsFor(() => {
        if (atom.workspace.getActivePaneItem()) {
          if (atom.notifications.getNotifications().length > 0) {
            return _.last(atom.notifications.getNotifications()).getMessage() === config.INFO_MESSAGES.REGISTERED_SUCCESSFULLY
          }
        }
      }, 'Timeout issue when registering in challenge', testConfig.TIMEOUT)
      runs(() => {
        // validate that button gets hidden
        expect(atom.workspace.getActivePaneItem().getElement().innerHTML)
          .toContain('<button id="registerButton" data-id="12345" style="display:none">')
      })
    })
  })
  it('cant register because it is already registered', () => {
    const rows = activePane.children[0].rows
    let index
    for (index = 0; index < rows.length; index++) {
      if (rows[index].cells.item(1).textContent === 'CODE') {
    ...
    runs(() => {
      expect(atom.workspace.getActivePaneItem().getElement().getElementsByTagName('button').length).toBe(0)
    })
  })
})
