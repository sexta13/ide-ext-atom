...
/**
 *
 * @param {String} html  Html where inline styling will be removed
 * @return {String} the new html
 */
const removeInlineStyleFromHtml = function (html) {
...
const generateHtmlFromChallenges = function (challenges) {
  return removeInlineStyleFromHtml(
    `<table border="1">
              <tr>
 ...
 */
const generateRegisterButtonHTML = function (challengeDetails, token) {
  const buttonHtml = `
      <button id="registerButton" data-id=${challengeDetails.challengeId}>
        Register
      </button>
      `
  // add register button to DOM only if this user has not already registered
  const registrants = _.get(challengeDetails, 'registrants', [])
